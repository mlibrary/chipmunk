...
module Chipmunk
  ...
  class IncomingStorage
    ...
    # @param volume [Volume] The Volume from which the deposited packages should be
    #   ingested
    # @param paths [PathBuilder] A PathBuilder that returns a path on disk to the user
    #   upload location for a deposit, for a given package.
    # @param links [PathBuilder] A PathBuilder that returns an rsync destination to
    #   which the user should upload, for a given package.
